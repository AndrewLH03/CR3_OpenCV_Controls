...
class Phase3IntegrationTest(Node):
    ...
    def run_test_sequence(self):
        """Run a test sequence"""
        self.get_logger().info("Starting Phase 3 integration test sequence...")
        ...
        # Summary
        self.get_logger().info(f"Test completed. Received {len(self.safety_alerts)} safety alerts")
        for i, alert in enumerate(self.safety_alerts):
            self.get_logger().info(f"Alert {i+1}: {alert.description}")
        
        return len(self.safety_alerts) > 0 or service_result
